...
;(function ($) {
  ...
  $.fn.placeholder = function () {
    ...
    var fnBlur = function () {
      if ($.trim(this.value) == '') {
        $(this)
                    .val('')
                    .parent()
                    .removeClass('ph-wrap-has ph-wrap-focus')
      }
    }
    return this.each(function () {
      ...
      $this
                .wrap(dWrap)
                .before(dSpan)
                .bind({ keyup: fnKey, blur: fnBlur, paste: fnPaste })
      if ($.trim(this.value) != '') {
        $this.parent().addClass('ph-wrap-has')
      }
    })
  ...
  window.onload = function () {
    ...
    // $('div[data-odd-even="odd-even"]:odd').css('background','#f6f6f6');
        // 今日主推 tab切换
    $('#command-brand .tabs a').live('click', function (e) {
      e.preventDefault()
      var $this = $(this), _id = $this.attr('href')
      $this.addClass('active').siblings().removeClass('active')
            // 品牌池
      $(_id).show().siblings('.panel').hide()

            // 品牌大图
      _id = _id.replace('#', '.')
      $(_id).show().siblings('.brand-pictures').hide()
    })

    $('#command-brand .panels>.panel').each(function (i, item) {
      var $item = $(item), _id = $item.attr('id')
      $item.ygSwitch('#brand-swiper' + (i + 1) + ' a', {
        nextBtn: '#' + _id + ' .arrow-right',
        prevBtn: '#' + _id + ' .arrow-left',
        steps: 9,
        visible: 9,
        effect: 'scroll'
      })
    ...
    $('ul.brand-pictures>li>a').hover(
        ...
        // 最新产品发布 tab切换
    $('#command-new-products .tabs a').live('click', function (e) {
      e.preventDefault()
      var $this = $(this), _id = $this.attr('href')
      $this.addClass('active').siblings().removeClass('active')
      $this.addClass('active').siblings().removeClass('active')
    ...
    $('#command-new-products .panels>.panel').each(function (i, item) {
      var $item = $(item), _id = $item.attr('id')
      $item.ygSwitch('#swiper' + (i + 1) + ' .swiper-sub-item', {
        nextBtn: '#' + _id + ' .arrow-right',
        prevBtn: '#' + _id + ' .arrow-left',
        steps: 4,
        visible: 4,
  ...
  YouGou.Util = {
    ...
    setHrefStamp: function (area) {
      ...
      target.each(function (index) {
        $(this).attr(
                    'href',
                    YouGou.Util.setUrlStamp($(this).attr('href'))
                )
      })
    }
  }
  ...
  YouGou.UI = {
    showTip: function (str, srcElementDom) {
      ...
      $('#yg-ui-showTip')
                ...
                .show()
                .animate({ left: offset.left - 50 })
      setTimeout(function () {
        $('#yg-ui-showTip').animate(
                    { left: offset.left, opacity: 0 },
                    function () {
                      $(this).hide()
                    }
                )
    ...
    ShoppingCart: {
      ...
      initCartCnt: function (data, isshow) {
        ...
        $('#shoppingcartContainer').html(data)
                // if(isshow){return;}
        $('#shoppingcartContainer li:first')
                    .find('.num_warntips')
                    .remove()
        ...
        $('.JsFavorite').click(function () {
          ...
          YouGou.Biz.ShoppingCart.addCommodityFavorite(
                        pid,
                        psize,
                        this
                    )
      ...
      showCart: function (isshow) {
        ...
        shopcartContainer.html(
                    '<div class="mycart_pop abs"><h3 class="hd rel">我的购物车</h3><div class="bd" style="padding:10px;text-align:center;width:324px;height:20px;"><img src="/images/loading.gif" /></div></div>'
                )
        $.ajax({
          type: 'POST',
          url: this.cartActionBasePath +
                        'getShoppingCartForHomePage.sc',
      ...
      changeProductNum: function (type, index, simpleShoppingCart) {
        ...
        $('input[name=' + name + ']').each(function () {
          sumProductNum =
                        parseInt(sumProductNum) + parseInt(this.value)
        })
        ...
        var param =
                    ...
                    productNo
        $.get(
                    this.cartActionBasePath + 'u_updateCart.sc',
                    param,
                    function (d) {
      ...
      removeProduct: function (id, isTip, srcElement) {
        ...
        var param =
                ...
                // 异步移除商品
        $.post(
                    this.cartActionBasePath + 'removeProduct.sc',
                    param,
                    function (d) {
                      if (srcElement) {
                        $(srcElement)
    ...
    WebsiteMessage: {
      init: function () {
        $.ajax({
          ...
          success: function (data) {
            if (data) {
              var count = data.substring(
                                data.lastIndexOf('count=') + 6,
                                data.lastIndexOf('msg=')
                            )
              var _msgHtml = []
              _msgHtml.push(
                                '<a href="https://www.yougou.com/my/msg.jhtml?' +
                                    YouGou.Util.timestamp() +
                                    '">站内消息(<i class="corg" >'
                            )
              _msgHtml.push(count)
              _msgHtml.push('</i>)</a>')
              ...
              if (count != 0) {
                try {
                  ...
                  $('#css_no_num').attr(
                                        'href',
                                        'https://www.yougou.com/my/msg.jhtml?' +
                                            YouGou.Util.timestamp()
                                    )
        ...
        $.get('/my/getCommentCount.jhtml', function (d) {
          var msg =
                        '<a href="https://www.yougou.com/my/comment.jhtml?' +
                        YouGou.Util.timestamp() +
                        '">等待点评(<i class="corg">' +
                        d +
                        '</i>)</a>'
...
function login () {
  location.href =
        'https://www.yougou.com/signin.jhtml?redirectURL=' +
...
function register () {
  location.href =
        'https://www.yougou.com/register.jhtml?redirectURL=' +
        encodeURI(location.href)
  return false
}
...
$(function () {
  ...
  ;(function () {
    function showImg () {
      $('#small-ad').css('display', 'none')
      var h = 450
      $('#big-ad').animate({ height: h }, 600, function () {
        window.setTimeout(function () {
          $('#big-ad').animate({ height: 0 }, 600, function () {
            $('#small-ad').css('display', 'block')
            leftScroll()
          })
        }, 2000)
      })
    ...
    if ($('#big-ad').length > 0) {
      ...
      if (bigShow_cookie != null) {
        if (cur_id == bigShow_cookie) {
          ...
          YouGou.Biz.cookie('bigShowState', cur_id, {
            expires: 0.25,
            path: '/',
            domain: '.yougou.com',
            secure: false
  ...
  $('#indexFocusPic .trgBox')
        .ygSwitch('#indexFocusPic .imgBox', {
          trigger: 'span',
          currCls: 'hover',
          circular: true,
          lazyload: true
  ...
  $('#uc_guessLike_box2')
        .ygSwitch('#uc_guessLike_box2>ul>li', {
          nextBtn: '#nextBtn2',
          prevBtn: '#prevBtn2',
          steps: 1,
          visible: 1,
        ...
        .carousel()
  $('#uc_guessLike_box')
        .ygSwitch('#uc_guessLike_box>ul>li', {
          nextBtn: '#nextBtn',
          prevBtn: '#prevBtn',
          steps: 1,
          visible: 1,
        ...
        .carousel()
  $('#uc_guessLike_box1')
        .ygSwitch('#uc_guessLike_box1>ul>li', {
          nextBtn: '#nextBtn1',
          prevBtn: '#prevBtn1',
          steps: 1,
          visible: 1,
  ...
  $('.mx-price a').mouseout(function () {
    ...
    // ajax登录
  var requestHost = document.location.protocol == 'https:'
        ? 'https://passport.yougou.com'
        : 'https://www.yougou.com'
  $.ajax({
    type: 'get',
    url: requestHost +
            '/api/getIsLoginAndShoppingcartJsonp.jhtml?callback=?',
    dataType: 'jsonp',
    success: function (data) {
      if (!data) return
      $('#pordcount').html(data.cartcount)
            // 登陆状态ntkf统计代码
      var dNtfj = $('#ntkfjs'), isCms = !!dNtfj[0]
      if (isCms) {
        NTKF_PARAM = {
          siteid: 'kf_9923',
      ...
      if (data.islogin != 'false' && data.islogin != false) {
        ...
        $('#top_nav .about_user').html(
                    '您好！<span id="user_id">' +
                        userName +
                        '</span><a class="quit" href="/logout.jhtml">退出</a>'
                )
                // tkf统计start
        if (isCms && NTKF_PARAM) {
    ...
    /** **图片延时加载***/
  $(
        '#indexFocusPic .imgBox:first img,#brand-list img:lt(48),.footser img,.n_footinfo img,.imglst img,.lazy_loading'
    ).lazyload(
    {
  ...
  $('.ind_md4 .tab')
        .ygSwitch('.ind_md4 .imglst', tabSwitchOption)
        .autoplay(tabtime)
  ...
  var domTopNav = $('#top_nav')
    // 我的优购 公告 更多 手机
  $('.my_yg,.notice,.more,.phone', domTopNav)
    ...
    $.fn.VSfocus = function (opts) {
      var ofuntion = {
        clear: function (o) {
          o
                        .removeClass(
                            'vsfocus vsfocusnumber vsfocustitle vsfocusthumb'
                        )
                        .find('.vsfocus_preNext')
                        .remove()
      ...
      if (typeof opts === 'string') {
        ...
        return this.each(function () {
          var o = $(this)
          o.data('opt', opt)
          var sWidth = $('div', o).length > 0
                        ? $('div', o).width()
                        : o.width(),
            sHeight = o.height() // 获取焦点图的宽度（显示面积）
          ...
          if (opt.isPre) {
            btn +=
                            '<div class="vsfocus_preNext vsfocus_pre" index="0">‹</div><div class="vsfocus_preNext vsfocus_next" index="0">›</div>'
          }
          if (opt.isTotal) {
          ...
          $('.vsfocus_pre', o).click(function () {
            if ($(this).hasClass('vsfocus_disabled')) {
              return
            }
          ...
          function showPics (index) {
            ...
            $('ul', o)
                            .stop(true, false)
                            .animate({ left: nowLeft }, 300) // 通过animate()调整ul元素滚动到计算出的position
            $('.vsfocus_btn li', o)
                            .stop(true, false)
                            .removeClass('vsfocus_btn_on')
                            .eq(index)
                            .addClass('vsfocus_btn_on')
          }
        })
      }
    }
