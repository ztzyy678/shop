...
// 购物袋去结算登陆成功回调(因登陆跨域回调问题,该方法只能放到全局变量)
/*
...
ygOrder = {
    ...
    module : {
                        ...
                        if (result.key == "0") {
                            ...
                            if (couponType == 1) {
                                ...
                                $(".qr_1").hover(function(){
                                    ...
                                    var erweima_html = '<div class="erweima_html"><img src="//s1.ygimg.cn/template/common/images/qrcode.png?3.3.5" width="136" height="136"</div>';
        ...
        shopcart : function(dModule) {
            ...
            // 删除购物袋货品
            // sid代表调用removeProduct的时候，是哪个购物袋调的
            ...
            function changeProductNum(type, index, sid) {
                ...
                // console.log(id);
                // console.log($("#" + sid).find('input').length);
                var newNumNs=0;
                $("#" + sid).find('input').each(
                    function() {
                        if(this.id !== null && this.id!== undefined &&this.id!== '' && this.type != 'checkbox'){
                            if(id === this.value){
                                // alert(this.id);
                                // console.log(this.id);
                                var id_index = this.id.slice(10);
                                // console.log('id_index='+id_index);
                                var newNumN = parseInt($("#" + sid + ' #oldNum_' + id_index).val());
                                newNumNs = newNumNs + newNumN;
                                // console.log(newNumN);
                            }
                        }
                    }
                ...
                // console.log(newNum);
                var newNum = type == "+" ? newNumNs + 1 : newNumNs - 1;
                // console.log(newNum);
                                    ...
                                    $.ajax({
                                        ...
                                        success : function(msg) {
                                            ...
                                            if (!$.isEmptyObject(_msg)) {
                                                ...
                                                ygDialog({
                                                    skin : 3,
                                                    content : "<p style='padding:0 20px;'>您的购物袋中尚有赠品可以领取</p><a class='uiClose spctbtnstyle' href='javascript:;'>领取</a><a href='"
                                                    + _continue_url
        ...
        formInit : function(dModule) {
            ...
            function submitOrder(form) {
                ...
                // 判断状态
                if ($('.jsShopCartErr').length > 0) {
                    alert('您所购买的商品中可能存在已下架、已售罄，或者库存不足的情况，请返回购物袋检查！');
                    $('body').scrollTop($('.jsShopCartErr').offset().top);
                    return false;
                }
                ;
                ...
                // 检查是否填写发票抬头
                if ($('.jsInvcComName').is(':visible')) {
                    var _ipttxt = $('.jsInvcComName');
                    var _val = _ipttxt.val();  //公司名称
                    var _errorTip = $('.jsInvcComp').find('.error_tipei');//公司名称提示信息
                    var TaxpayerNum =$("#TaxpayerNum").val();//纳税人识别号
                    var num_errorTip = $('.TaxpayerNum').find('.error_tipei');//纳税人识别号提示信息
                    var regular = /^(?!0+$)[A-Za-z0-9]{15,20}$/;//字母和数字，最大长度18位的正则
                    if (_val == "") {
                        _errorTip.css('display', 'inline-block').html('请输入公司名称');
                        return false;
                    }else{
                        _errorTip.hide();
                    }
                    if(TaxpayerNum =="" || !regular.test(TaxpayerNum)){
                        num_errorTip.css('display', 'inline-block').html('请输入正确的纳税人识别号');
                        return false;
                    }else{
                        num_errorTip.hide();
                        companyInvoice = 1;//1代表：单位发票类型
                    }
                ...
                if ($('#hasZeroProduct').val() == '1') {
                    alert("订单存在实付金额为0元的商品，将无法清关，请调整购物袋或分开结算");
                    return false;
                }
...
function showSimilarReconmmendList(_data) {
    if (_data && _data.length > 0) {
        ...
        for (var i = 0; i < _data.length; i++) {
            ...
            sHtml.push("<p class=\"mt15\"><a href=\"javascript:;\" class=\"cart_addto_lnk\" goodsno='"+_data[i].goodsno+"'>加入购物袋</a></p></div>");
            if (_data[i].has2=='0') {
                ...
                sHtml.push("<p class=\"mt15\"><a href=\"javascript:;\" class=\"cart_addto_lnk\" goodsno='"+_data[i].goodsno2+"'>加入购物袋</a></p>");
                sHtml.push("</div>");
            }
            sHtml.push("</div></li>");
        }
...
(function($) {
    ...
    var oEInvoice = {
        ...
        /*发票类型---个人*/
        personFP:function(){
            var radioValOne = $("input[id='pp']:checked").val();
            if(radioValOne=="0"){
                $(".companyMsgs").hide();
                $("#eInvoiceCompanyName").val('');
                $("#eInvoiceCompanyUserName").val('');
                $("#invoiceTitleType").val('0');
            }
        },
        /*发票类型---单位*/
        companyFP:function(){
            var radioValTwo = $("input[id='pp_company']:checked").val();
            if(radioValTwo=="0"){
                $(".companyMsgs").show();
                $("#invoiceTitleType").val('1');//1代表“单位”发票类型
                $("#eInvoiceCompanyName").attr("name","eInvoiceTitle");//修改单位发票的公司名称input的name值
                $("#TaxpayerNum").attr("name","taxRegisterNo");//修改单位发票的纳税识别号input的name值
            }
        },
        /* 验证手机号 */
        fValidMobilePhone : function(showAlert) {
            ...
            if (p == "") {
                ...
                if(eInAddr==""||eInAddr==null){
                    var mpr = "^(13[0-9]|14[5|7]|15[0|1|2|3|5|6|7|8|9]|18[0-9]|17[0-9]|19[8|9]|166)\\d{8}$";
                    isValid = p.match(mpr);
                    if (!isValid) {
                        /* 手机号格式不正确 */
                        msg = $("#eInvoiceInvalidMsg2");
